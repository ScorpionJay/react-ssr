...
class Play extends Component {
  ...
  render() {
    ...
    let krc2 = currentMusic.krc.filter(item => currentTime > item.time);
    let s = krc2.pop();
    s = s
      ? s
      : {
      ...
      <div style={{ display: "flex" }}>
        ...
        >
          ...
          <div
            ...
            onClick={() => this.setState({ playList: false })}
            ref="krcContainer"
          >
                ...
                style={Object.assign(
                  {
                    transform:
                      "translateY(" +
                      (this.state.krcHeight -
                        (s.index + 1) * this.state.krcItem) +
                      "px)",
                    transition: "transform .5s ease",
                    padding: "1rem 0",
                    height: "2rem"
                  },
                  s.time === item.time ? { color: "#fff" } : {}
                )}
                ref="krcItem"
              >
                {item.str}
          ...
          <div style={{ padding: "1rem" }}>
            <div style={{ display: "flex" }}>
              <div style={{ padding: "0 1rem", color: "#fff" }}>
                {" "}
                {Format.formatSeconds(currentTime)}{" "}
              </div>
              ...
              <div style={{ padding: "0 .5rem", color: "#fff" }}>
                {" "}
            ...
            >
              ...
              <div
                onClick={() =>
                  this.props.dispatch(
                    changeMusicAPI(musicPlayList, currentMusic, "pre")
                  )
                }
              >
                <PreBtn />
              </div>
              <div onClick={() => this.musicControll()}>
                {controll === "play" ? <StopBtn /> : <PlayBtn />}
              </div>
              <div
                onClick={() =>
                  this.props.dispatch(
                    changeMusicAPI(musicPlayList, currentMusic)
                  )
                }
              >
                <NextBtn />
              </div>
              <div onClick={() => this.setState({ playList: true })}>
                <ListBtn />
              </div>
            </div>
          </div>
          ...
          <div
            ...
            style={Object.assign(
              ...
              this.state.playList ? { display: "block" } : { display: "none" }
            )}
          >
            <div
              style={{
            ...
            >
              ...
              >
                播放列表{" "}
                {currentMusic.hash === "" ? "(0)" : `(${musicList.length})`}
              </div>
              {musicList.map((item, index) => (
                <div
                  key={index}
                  style={
                    currentMusic.hash === item.hash ? { color: "#ce3d3e" } : {}
                  }
                >
